...
struct Passenger bookedTickets[100];
...
void displayBusSchedules() {
    ...
    for (int i = 0; i < sizeof(busRoutes) / sizeof(busRoutes[0]); i++) {
...
void bookTicket() {
    int routeChoice;
    int numRoutes = sizeof(busRoutes) / sizeof(busRoutes[0]);
...
void displayTickets() {
    ...
    for (int i = 0; i < numBookedTickets; i++) {
        int busId = bookedTickets[i].busId;
        if (busId >= 1 && busId <= sizeof(busRoutes) / sizeof(busRoutes[0])) {
            printf("%-15s | %-20s | ksh%.2f   | %s | %d\n",
                bookedTickets[i].name, busRoutes[busId - 1].name, busRoutes[busId - 1].price, busRoutes[busId - 1].schedule, bookedTickets[i].seatNumber);
        }
    }
}
